...
- [X] T038 [US3] Edit `config/routes.rb` — add `resources :transactions` (the `export_csv` collection route will be added in Phase 8)
...
- [X] T040 [US3] Create `app/views/transactions/new.html.erb` — page title "New Transaction" and render `form` partial
- [X] T041 [US3] Create `app/views/transactions/edit.html.erb` — page title "Edit Transaction" and render `form` partial
...
- [X] T046 [US5] Edit `config/routes.rb` — replace or confirm `resource :dashboard, only: [:show], controller: "dashboard"` and the `authenticated :user do root "dashboard#show" end` block are present
